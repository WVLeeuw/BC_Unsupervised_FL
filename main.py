...
if __name__ == '__main__':
    ...
    devices_in_network = DevicesInNetwork(is_iid=args['IID'], num_devices=num_devices, num_malicious=num_malicious,
                                          ...
                                          leader_wait_time=args['leader_wait_time'],
    ...
    for comm_round in range(latest_round_num + 1, args['num_comm'] + 1):
        ...
        sorted_beta_samples = {k: v for k, v in sorted(beta_samples.items(), key=lambda item: item[1], reverse=True)}
        # take top n for n = committee + leaders, shuffle them and assign them.
        sampled_devices = dict(list(sorted_beta_samples.items())[:committee_members_to_assign+leaders_to_assign])
        sampled_devices_keys = list(sampled_devices.keys())  # extract only the device idxs.
        ...
        for comm_member in committee_members_this_round:
            ...
            # ToDo: consider local computation power (for validation) here and delete code pertaining to simple avg.
            # updates_per_centroid = comm_member.match_local_with_global_centroids()  # currently, not being used.
            # aggr_centroids = comm_member.aggr_updates(updates_per_centroid)  # currently, not being used
        ...
        if winning_block:
            ...
            for comm_member in committee_members_this_round:
                pass
                # average the performances found (silhouette) per local update and compare it to some predefined
                # parameter (threshold) to determine whether we want to re-initialize if there is no winning block.
                # If it is above that threshold, we do not care that there is no winning block during that round.
            pass
